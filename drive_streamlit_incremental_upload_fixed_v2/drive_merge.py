...
def ensure_folder_access(drive, folder_id: str):
    """
    Verify folder exists and service account can access it.
    """
    ...
    if meta.get("mimeType") != FOLDER_MIME:
        raise ValueError(f"ID này không phải folder: {folder_id} (mimeType={meta.get('mimeType')})")
    return meta
...
def upload_streamlit_file_to_drive(drive, folder_id: str, uploaded_file) -> str:
    ...
    Dùng upload **không resumable** (MediaInMemoryUpload) để tránh lỗi ResumableUploadError
    do Streamlit UploadedFile đôi khi không tương thích seek/chunk trong chế độ resumable.
    """
    ensure_folder_access(drive, folder_id)

    filename = uploaded_file.name
    # Streamlit UploadedFile thường có .type (mimetype)
    mime = getattr(uploaded_file, "type", None) or mimetypes.guess_type(filename)[0] or "application/octet-stream"
