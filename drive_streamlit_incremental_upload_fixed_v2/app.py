...
if run_btn:
    ...
    try:
        ...
        # Hiển thị lỗi rõ hơn (tránh bị Streamlit redact toàn bộ)
        msg = str(e)
        try:
            # ResumableUploadError có resp/content
            if hasattr(e, 'content') and e.content:
                msg += "\n" + e.content.decode('utf-8', errors='ignore')
        except Exception:
            pass
        st.error("""Upload thất bại. Hãy kiểm tra:
- Service account được share **Editor** cho RAW/ARCHIVE/EXTRACT folder
- RAW_FOLDER_ID đúng là folder (không phải file)
- Nếu dùng Shared Drive: service account phải là member của Shared Drive
